...
#import "CDVSTModal.h"
#import "CDVSTModalUtil.h"

#define kSTAlertWidth        Main_Width - 50
#define kSTAlertPaddingV     8
#define kSTAlertPaddingH     20
#define kSTAlertRadius       13
#define kSTAlertButtonHeight 40

@interface CDVSTAlertView ()
...
- (void)loadData{
    ...
    _modal = [CDVSTModal modalWithContentView:self];
...
- (void)loadUI{
    ...
    _backgroundView.backgroundColor = STModalRGBA(250, 250, 250, 1);
...
- (void)addLabel:(UILabel *)label maxHeight:(CGFloat)maxHeight{
    ...
    label.frame = CGRectMake(kSTAlertPaddingH, _scrollBottom+8, _maxContentWidth, size.height);
...
- (void)loadImage{
    ...
    _imageView.frame = CGRectMake(kSTAlertPaddingH+_maxContentWidth/2-size.width/2,
                                  _scrollBottom, size.width, size.height);
...
- (UIButton *)addButton:(CGRect)frame title:(NSString *)title tag:(NSInteger)tag {
    ...
    button.tag = tag;
    UIColor *color = [UIColor colorWithRed:68.0/255.0 green:68.0/255.0 blue:68.0/255.0 alpha:1];
...
@implementation CDVSTAlertView (Show)
...
+ (instancetype)showTitle:(NSString *)title
    ...
    CDVSTAlertView *alert = [[CDVSTAlertView alloc] initWithTitle:title
                                                      image:image
                                                    message:message
...
+ (instancetype)showTitle:(NSString *)title
    ...
    if (delay>0){
        CDVSTAlertView *alert = [self showTitle:title message:message];
